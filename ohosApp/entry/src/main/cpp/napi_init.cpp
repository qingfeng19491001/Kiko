...
using BridgeRef = libshared_kref_com_kuikly_stockchat_kline_StockOhosKLineBridge;
...
static napi_value CreateKLineBridge(napi_env env, napi_callback_info) {
    ...
    bridges.emplace(handle, SharedApi()->kotlin.root.com.kuikly.stockchat.kline
        .StockOhosKLineBridge.StockOhosKLineBridge());
    napi_value result;
...
static napi_value SetKLineProp(napi_env env, napi_callback_info info) {
    ...
    const bool handled = SharedApi()->kotlin.root.com.kuikly.stockchat.kline
        .StockOhosKLineBridge.setProp(*bridge, key.c_str(), value.c_str());
    napi_value result;
...
static napi_value CallKLine(napi_env env, napi_callback_info info) {
    ...
    return JsString(env, SharedApi()->kotlin.root.com.kuikly.stockchat.kline
        .StockOhosKLineBridge.call(*bridge, method.c_str(), params.c_str()));
}
...
static napi_value ResizeKLine(napi_env env, napi_callback_info info) {
    ...
    SharedApi()->kotlin.root.com.kuikly.stockchat.kline
        .StockOhosKLineBridge.resize(*bridge, width, height);
...
static napi_value PointerKLine(napi_env env, napi_callback_info info) {
    ...
    SharedApi()->kotlin.root.com.kuikly.stockchat.kline
        .StockOhosKLineBridge.pointer(*bridge, kind.c_str(), x, y, scale, count);
    return Undefined(env);
}
...
static napi_value RenderKLineCommands(napi_env env, napi_callback_info info) {
    ...
    if (bridge == nullptr) return Undefined(env);
    return JsString(env, SharedApi()->kotlin.root.com.kuikly.stockchat.kline
        .StockOhosKLineBridge.renderCommands(*bridge));
}
...
static napi_value PollKLineEvents(napi_env env, napi_callback_info info) {
    ...
    if (bridge == nullptr) return Undefined(env);
    return JsString(env, SharedApi()->kotlin.root.com.kuikly.stockchat.kline
        .StockOhosKLineBridge.pollEvents(*bridge));
}
...
static napi_value DisposeKLineBridge(napi_env env, napi_callback_info info) {
    ...
    if (found != bridges.end()) {
        SharedApi()->kotlin.root.com.kuikly.stockchat.kline
            .StockOhosKLineBridge.dispose(found->second);
        bridges.erase(found);
    }
    return Undefined(env);
}
