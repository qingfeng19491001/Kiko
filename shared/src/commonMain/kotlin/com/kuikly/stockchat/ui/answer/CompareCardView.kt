...
fun ViewContainer<*, *>.CompareCardView(
    block: AnswerBlock.CompareCard,
    contentWidth: Float,
    onOpen: (key: String) -> Unit,
    tableState: StockTableState? = null,
) {
    val state = tableState ?: StockTableState()
    state.ensure(compareTableColumns(block.leftName, block.rightName), compareTableRows(block))
    Card(padding = 0f) {
        ...
        View {
            ...
            StockQuoteTable(
                ...
                onCellClick = { _, columnKey, _ ->
                    when (columnKey) {
                        "left" -> onOpen(block.leftKey)
                        "right" -> onOpen(block.rightKey)
                    }
                },
            )
        }
