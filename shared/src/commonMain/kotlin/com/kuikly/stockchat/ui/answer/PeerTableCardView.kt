...
fun ViewContainer<*, *>.PeerTableCardView(
    block: AnswerBlock.PeerTableCard,
    contentWidth: Float,
    onOpen: (key: String) -> Unit,
    tableState: StockTableState? = null,
) {
    val state = tableState ?: StockTableState()
    state.ensure(peerTableColumns(), peerTableRows(block.rows))
    Card(padding = 0f) {
        ...
        View {
            ...
            StockQuoteTable(
                ...
                onCellClick = { _, _, row ->
                    if (row.id.isNotEmpty()) onOpen(row.id)
                },
            )
        }
    }
