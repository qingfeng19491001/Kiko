...
fun ViewContainer<*, *>.CapitalFlowCardView(
    block: AnswerBlock.CapitalFlowCard,
    contentWidth: Float,
    tableState: StockTableState? = null,
) {
    val state = tableState ?: StockTableState()
    state.ensure(capitalFlowColumns(), capitalFlowRows(block.flows))
    Card(padding = 0f) {
        ...
        View {
            ...
            StockQuoteTable(state, contentWidth - 8f, block.flows.size)
        }
    }
}
