...
fun ViewContainer<*, *>.SeriesChartCardView(
    ...
    val state = chartState ?: StockChartState()
    state.ensure(seriesChartData(block))
    ...
    Card(padding = 14f) {
        ...
        View {
            attr { marginTop(6f) }
            when (block.kind) {
                SeriesChartKind.LINE -> StockLineChart(state, chartW, 188f, "", block.unit)
                SeriesChartKind.GROUPED_BAR -> StockBarChart(state, chartW, 188f, "", block.unit)
            }
        }
    }
}
