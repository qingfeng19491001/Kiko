...
object IntentParser {
    ...
    private val rankKeywords = listOf("排名", "同业", "同行业", "行业里", "行业中", "地位", "对标", "赛道", "板块里", "板块中")
    private val trendKeywords = listOf(
        "趋势", "走势", "技术面", "均线", "k线", "K线", "形态", "突破", "支撑", "压力",
        "短期", "中期", "判断", "后市", "怎么走", "会怎么走",
    )
    ...
    private val analysisKeywords = listOf("诊股", "如何", "怎么看", "怎么样", "分析", "值得", "买", "卖", "持有", "行情", "估值", "怎样", "能不能", "可以", "全面", "综合")
    private val greetingKeywords = listOf("你好", "hello", "hi", "在吗", "你是谁", "介绍一下你", "能做什么", "帮我做什么", "技能广场")
    ...
    fun parse(text: String, contextInstruments: List<Instrument> = emptyList()): ParsedIntent {
        ...
        val hasRank = rankKeywords.any { lower.contains(it.lowercase()) }
        ...
        if (instruments.isEmpty() && contextInstruments.isNotEmpty() && (hasTrend || hasRisk || hasAnalysis || hasCompare || hasRank || hasFlow)) {
            instruments = if (hasCompare && !hasRank) contextInstruments.take(2) else contextInstruments.take(1)
        }
        ...
        val intent = when {
            ...
            instruments.size >= 2 && (hasCompare || hasRank || !hasKnowledge) -> Intent.COMPARE
            hasRank && instruments.isNotEmpty() -> Intent.COMPARE
